...
def extract_medications_features(baseline_df, chunk_size=100000):
    """Extract medication features with updated time windows"""
    print("\n" + "="*60)
...
def extract_icd_features(baseline_df, chunk_size=100000):
    """Extract ICD code/comorbidity features with time windows"""
    print("\n" + "="*60)
...
def save_enhanced_data(enhanced_df):
    ...
    print(f"Feature columns: {', '.join(enhanced_df.columns.tolist())}")
...
def create_medication_visualization(df):
    """Create medication visualization with counts AND percentages on bars"""
    print("\nCreating medication visualization with counts and percentages...")
    ...
    bars1 = ax.bar(x - width, at_diag, width, label='At Diagnosis (within 6 months)', 
                   color='#3498db', edgecolor='black', alpha=0.8)
    bars2 = ax.bar(x, two_year, width, label='2 Years (2-3 years, closest to 2)', 
                   color='#e67e22', edgecolor='black', alpha=0.8)
    bars3 = ax.bar(x + width, five_year, width, label='5 Years (4-6 years, closest to 5)', 
                   color='#9b59b6', edgecolor='black', alpha=0.8)
    
    ax.set_ylabel('Number of Patients', fontsize=12)
    ax.set_title(f'Medication Class Usage by Time Frame (N={total_n:,})', 
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, 'Medication_Classes.png'), 
                bbox_inches='tight', dpi=300)
    plt.close()
    print("  Medication visualization saved!")


def create_comorbidity_visualization(df):
    """Create comorbidity visualization with counts AND percentages on bars"""
    print("\nCreating comorbidity visualization with counts and percentages...")
    ...
    ax.set_ylabel('Number of Patients', fontsize=12)
    ax.set_title(f'Comorbidity Occurrence by Time Frame (N={total_n:,})', 
                 fontsize=14, fontweight='bold')
    ax.set_xticks(x)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, 'Comorbidities.png'), 
                bbox_inches='tight', dpi=300)
    plt.close()
...
def create_visualizations(df):
    ...
    # Call new consolidated visualization functions
    create_medication_visualization(df)
    create_comorbidity_visualization(df)
    create_race_ethnicity_visualization(df)
    
    # 1. Demographics plots - IMPROVED
    print("\nCreating demographics plots...")
    fig, axes = plt.subplots(2, 2, figsize=(15, 12))
    fig.suptitle('Demographics Distribution', fontsize=16, fontweight='bold')
    
    # Age distribution - ADD LABELS
    if 'AgeatDiagnosis' in df.columns:
        age_data = df['AgeatDiagnosis'].dropna()
    ...
    # Gender distribution - already has labels
    if 'gender' in df.columns:
        gender_counts = df['gender'].value_counts()
    ...
    # Race distribution (top 10) - ADD LABELS
    ...
    # Ethnicity distribution - already has labels
    ...
    # 3. Antibody status plots - KEEP AS IS (ALREADY GOOD)
...
def generate_analysis_and_visualizations():
    ...
    print("  - Medication_Classes.png")
    print("  - Comorbidities.png")
...
def main():
    ...
    # NEW: Create composite race/ethnicity groups
    enhanced_df = create_race_ethnicity_composite(enhanced_df)
    
    enhanced_df = extract_medications_features(enhanced_df)
    enhanced_df = extract_icd_features(enhanced_df)
    save_enhanced_data(enhanced_df)
    generate_analysis_and_visualizations()

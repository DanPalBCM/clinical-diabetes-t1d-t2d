...
def add_growth_percentiles(df, time_point, measurement, cdc_table, who_table):
    ...
    required_cols = [value_col, age_col, "gender_numeric"]
    ...
    # ========================================================================
    # Join with reference tables: match on gender, then find nearest age
    # ========================================================================
    
    # Join with WHO table - match on gender only for children < 24 months
    df_with_who = df.join(
        who_prepared,
        (F.col("gender_numeric") == F.col(f"who_sex_{time_point}_{measurement}")) &
    ...
    # Join with CDC table - match on gender only for children >= 24 months
    df_with_both = df_with_who.join(
        cdc_prepared,
        (F.col("gender_numeric") == F.col(f"cdc_sex_{time_point}_{measurement}")) &
    ...
    if measurement == "weight_kg" and time_point == "diagnosis":
        ...
        df_with_both = df_with_both.withColumn(
            ...
            F.col("gender_numeric")
        )
        
        df_with_both = df_with_both.withColumn(
            "_debug_gender_text",
            F.when(F.col("gender_numeric") == 1, F.lit("MALE"))
             .when(F.col("gender_numeric") == 2, F.lit("FEMALE"))
...
@transform(
    ...
    input_measurements=Input("ri.foundry.main.dataset.xxxxx"), # Output from step 2
...
def compute(input_measurements, output_dataset,
    ...
    # ========================================================================
    # Convert gender to numeric codes (1=Male, 2=Female) - ONCE for entire dataframe
    # ========================================================================
    df = df.withColumn(
        "gender_numeric",
        F.when(F.upper(F.col("gender")) == "MALE", 1)
         .when(F.upper(F.col("gender")) == "FEMALE", 2)
         .when(F.upper(F.col("gender")) == "M", 1)
         .when(F.upper(F.col("gender")) == "F", 2)
         .otherwise(None)
    )
    ...
    for time_point in ["diagnosis", "2_years", "5_years"]:
        ...
        # ====================================================================
        # Convert weight from OUNCES (oz_av) to kg
        ...
        if f"weight_at_{time_point}" in df.columns:
            df = df.withColumn(
                ...
                F.when(
                    ...
                    F.col(f"weight_at_{time_point}").cast("double") * 0.028349523125
                )
            )

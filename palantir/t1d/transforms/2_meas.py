...
@transform(
    ...
    t2d_enhanced_omop=Input("ri.foundry.main.dataset.xxxxx"), # T1D/T2D input data from AWS list of patients
...
def compute(t2d_enhanced_omop, omop_phi_measurement, output_dataset):
    ...
    t2d_patients = (
        ...
        .select(F.col("person_id").cast("long").alias("PERSON_ID"))
        .distinct()
    )
    ...
    categorized_measurements = measurements_with_upper.withColumn(
        ...
        .when(
            # Total Cholesterol - exclude ratios and non-total variations
            (F.col("measurement_upper").contains("CHOLESTEROL") |
             F.col("measurement_upper").contains("CHOL")) &
            ~F.col("measurement_upper").contains("HDL") &
            ~F.col("measurement_upper").contains("LDL") &
            ~F.col("measurement_upper").contains("VLDL") &
